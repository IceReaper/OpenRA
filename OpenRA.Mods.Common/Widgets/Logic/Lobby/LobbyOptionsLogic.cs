	...
	public class LobbyOptionsLogic : ChromeLogic
	{
		...
		void RebuildOptions()
		{
			...
			var allOptions = mapPreview.Rules.Actors["player"].TraitInfos<ILobbyOptions>()
					...
					.ToDictionary(e => e.Key, e => e.OrderBy(o => o.DisplayOrder).ToArray())
					.OrderBy(e => e.Key)
					.ToArray();

			foreach (var groupOptions in allOptions)
			{
				Widget row = null;
				...
				if (groupOptions.Key != "")
				{
					var headline = headlineTemplate.Clone();
					((LabelWidget)headline).GetText = () => groupOptions.Key;
				...
				foreach (var option in groupOptions.Value.Where(o => o is LobbyBooleanOption))
				{
